        ...
        while i < data.len() {
            ...
            match op {
                ...
                OpCode::Function => {
                    ...
                    let plvs_ptr = self.local_var_stack.as_ptr() as *const _ as u64;
                    {
                        match value_rr {
                            Value::String(ref sp) => {
                                let s = &sp.borrow().s;
                                match chunk_functions.borrow().get(i2 as usize) {
                    ...
                    if copy {
                        ...
                        let cv_value_rr = chunk_functions.borrow().get(i2 as usize).unwrap().clone();
                        match cv_value_rr {
                ...
                OpCode::Constant => {
                    ...
                    {
                        let cvb = chunk_functions.borrow();
                        let iv = cvb.get(i2 as usize);
                        if self.debug {
                            eprintln!("CFP: {:?}", iv);
                        }
                        match iv {
