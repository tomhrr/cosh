...
impl VM {
    ...
    /// Get the union of two IP sets.
    pub fn core_ips_union(&mut self) -> i32 {
        if self.stack.len() < 2 {
            self.print_error("ips.union requires two arguments");
            return 0;
        }
    ...
    /// Get the intersection of two IP sets.
    pub fn core_ips_isect(&mut self) -> i32 {
        if self.stack.len() < 2 {
            self.print_error("ips.isect requires two arguments");
            return 0;
        }
    ...
    /// Get the difference between two IP sets (i.e. A less B).
    pub fn core_ips_diff(&mut self) -> i32 {
        if self.stack.len() < 2 {
            self.print_error("ips.diff requires two arguments");
            return 0;
        }
        ...
        match (ipset1_rr, ipset2_rr) {
            (Value::IpSet(ipset1), Value::IpSet(ipset2)) => {
                ...
                let ipset2_ipv4 = ipset2.ipv4;
                let ipset2_ipv6 = ipset2.ipv6;
                let new_ipv4 = ipset2_ipv4.exclude(&ipset1_ipv4);
                let new_ipv6 = ipset2_ipv6.exclude(&ipset1_ipv6);
                let new_ipset = IpSet::new(new_ipv4, new_ipv6);
                self.stack.push(Value::IpSet(new_ipset));
                return 1;
            }
    ...
    /// Get the symmetric difference between two IP sets (i.e. the union of A and
    /// B less the intersection of A and B).
    pub fn core_ips_symdiff(&mut self) -> i32 {
        if self.stack.len() < 2 {
            self.print_error("ips.symdiff requires two arguments");
            return 0;
        }
